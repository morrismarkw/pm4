...
const generalTopics = [
    { topic: 'Overview', term: 'Project', description: 'The Project helps you track and manage the overall scope, budget, and time allocation for a project. It consolidates important metrics such as total budget, planned and utilized hours, and resource allocation. With detailed fields for duration, effective rates, and status indicators, this provides a comprehensive view of project progress, making it easier to monitor and adjust resources and timelines as needed.', object: 'Project' },
    { topic: 'Overview', term: 'Project Budgets', description: 'The Project Budgets help you allocate and track labor hours based on the dollar amounts from associated Opportunity Product Line Items. These amounts are converted into blended hours, which are evenly distributed across the project timeline, making it easier to manage and compare your planned labor hours against actual time spent.', object: 'Project Budget' },
    { topic: 'Overview', term: 'Project Roles', description: 'The Project Roles help you define and manage roles within a project, specifying planned labor for each role. By default, the planned hours are spread uniformly across the project’s timeline, but you can make manual adjustments as needed, making it easier to manage and compare your planned labor hours against both the project budget and actual time spent.', object: 'Project Roles' },
    { topic: 'Overview', term: 'Project Resources', description: 'The Project Resources help you define and manage resources within a project, specifying planned labor for each resource. By default, the planned hours are spread uniformly across the project’s timeline, but you can make manual adjustments as needed, making it easier to manage and compare your planned labor hours against both the project budget and actual time spent.', object: 'Project Resources' },
    { topic: 'Overview', term: 'Project Tasks', description: 'The Project Tasks help you manage specific tasks within a project that resources will use to bill their time. Tasks can be designated as billable or unbillable, providing a clear structure for tracking time entries and ensuring accurate billing and time management throughout the project.', object: 'Project Tasks' },
    { topic: 'Overview', term: 'Resources', description: 'The Resources help you track and manage individuals or teams who are assigned to projects. You can specify availability, assign them to roles and projects, and track their time utilization across various tasks and projects.', object: 'Resource' },
    { topic: 'Overview', term: 'Roles', description: 'The Roles help you categorize and manage different responsibilities and functions within your projects. You can define roles for resources, allocate planned hours, and track the performance of each role against the project’s goals.', object: 'Role' }
];
...
export default class HelpTable extends LightningElement {
    @track helpData = [];
    ...
    connectedCallback() {
        this.helpData = addUniqueIds(helpData);
        this.filteredHelpData = this.helpData;
        this.populatePicklistOptions();
    }

    populatePicklistOptions() {
        const topics = [...new Set(this.helpData.map(item => item.topic))];
        const terms = [...new Set(this.helpData.map(item => item.term))];
        
        this.topicOptions = [{ label: 'All Topics', value: '' }, ...topics.map(topic => ({ label: topic, value: topic }))];
        this.termOptions = [{ label: 'All Terms', value: '' }, ...terms.map(term => ({ label: term, value: term }))];
    }
    ...
    filterData() {
        this.filteredHelpData = this.helpData.filter(item => {
            const topicMatch = this.selectedTopic ? item.topic === this.selectedTopic : true;
            const termMatch = this.selectedTerm ? item.term === this.selectedTerm : true;
    ...
    handleSort(event) {
        ...
        const cloneData = [...this.filteredHelpData];
        cloneData.sort(this.sortBy(sortedBy, sortedDirection === 'asc' ? 1 : -1));
        this.filteredHelpData = cloneData;
